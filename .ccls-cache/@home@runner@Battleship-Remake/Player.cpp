...
Player::Player() {
    ...
    // set the number of unsunk ships to be the total available ships 
    shipsLeft = totalShips;

    // set up myShips to be an array of null pointers to indicate that no ships have been placed yet
...
void Player::beginGame() {
    ...
    for(int i = 0; i < totalShips; i++){
        setUpShips(availShips[i]->getShipType(), availShips[i]->getSize(), i);
    }
